...
def load_sgdet_detection(
    ...
    *,
    ...
    _restore_required_detection_fields(box, cfg=cfg)
    return box


def _restore_required_detection_fields(
    box: BoxList,
    *,
    cfg: Mapping[str, Any],
) -> None:
    """Normalize cached proposals to the live detector's field contract.

    Detector images with no surviving proposal are valid.  Older v5 cache
    construction serialized such a BoxList with an empty ``fields`` mapping,
    while the live empty-detector route attaches correctly shaped empty
    tensors.  Downstream SGDet label assignment still expects those fields.

    For non-empty proposals we only reconstruct values from detector evidence
    already present in the cache (logits or detector-NMS labels/scores).  A
    genuinely corrupt non-empty entry remains an error rather than silently
    fabricating predictions or falling back to the raw detector.
    """

    model_cfg = cfg.get("MODEL", {})
    num_classes = int(
        model_cfg.get("ROI_BOX_HEAD", {}).get(
            "NUM_CLASSES", model_cfg.get("NUM_CLASSES", 0)
        )
    )
    if num_classes <= 1:
        raise ValueError(f"Invalid detector class count for SGDet cache: {num_classes}")
    device = box.bbox.device
    count = len(box)

    logits = None
    if box.has_field("predict_logits"):
        logits = box.get_field("predict_logits")
    elif box.has_field("pred_logits"):
        logits = box.get_field("pred_logits")
        box.add_field("predict_logits", logits)
    elif count == 0:
        logits = box.bbox.new_zeros((0, num_classes))
        box.add_field("predict_logits", logits)
        box.add_field("pred_logits", logits)

    if logits is not None:
        if tuple(logits.shape) != (count, num_classes):
            raise RuntimeError(
                "Invalid cached detector-logit shape: "
                f"expected={(count, num_classes)}, actual={tuple(logits.shape)}"
            )
        if not box.has_field("pred_logits"):
            box.add_field("pred_logits", logits)

    if not box.has_field("pred_labels"):
        if logits is not None and count > 0:
            probabilities = torch.softmax(logits, dim=1)
            pred_scores, labels = probabilities[:, 1:].max(dim=1)
            box.add_field("pred_labels", labels.long() + 1)
            if not box.has_field("pred_scores"):
                box.add_field("pred_scores", pred_scores)
        elif box.has_field("detector_nms_labels"):
            labels = box.get_field("detector_nms_labels").long() + 1
            box.add_field("pred_labels", labels.clamp(max=num_classes - 1))
        elif count == 0:
            box.add_field(
            ...
            raise RuntimeError(
                "Non-empty SGDet cache entry has no pred_labels, detector "
                "logits, or detector_nms_labels"
            )

    if not box.has_field("pred_scores"):
        if logits is not None and count > 0:
            pred_scores = torch.softmax(logits, dim=1)[:, 1:].max(dim=1).values
        elif box.has_field("detector_nms_scores"):
            pred_scores = box.get_field("detector_nms_scores").float()
        elif box.has_field("scores"):
            pred_scores = box.get_field("scores").float()
        elif count == 0:
            pred_scores = box.bbox.new_zeros((0,))
        else:
            raise RuntimeError(
                "Non-empty SGDet cache entry has no detector prediction scores"
            )
        box.add_field("pred_scores", pred_scores)

    if not box.has_field("scores"):
        box.add_field("scores", box.get_field("pred_scores"))
    if not box.has_field("detector_nms_labels"):
        box.add_field(
            "detector_nms_labels",
            (box.get_field("pred_labels").long() - 1).clamp(min=0),
        )
    if not box.has_field("detector_nms_scores"):
        box.add_field("detector_nms_scores", box.get_field("pred_scores"))
    if not box.has_field("boxes_per_cls"):
        box.add_field(
            "boxes_per_cls",
            box.bbox[:, None, :].expand(-1, num_classes, -1).clone(),
        )
