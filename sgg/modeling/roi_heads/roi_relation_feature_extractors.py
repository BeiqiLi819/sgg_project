...
class RelationUnionFeatureExtractor(nn.Module):
    def __init__(self, cfg: dict, in_channels: int):
        ...
        self.out_channels = hidden_dim
        # Read-only diagnostic capture used by the frozen-evidence probes.  It
        # is deliberately opt-in so ordinary training/evaluation does not keep
        # an additional reference to the (potentially large) ROI tensor.
        self.round11_feature_capture_enabled = False
        self._round11_pure_union_visual = None

    def round11_pure_union_visual(self):
        """Return the last parameter-free pooled pre-rectangle union tensor."""

        return self._round11_pure_union_visual

    def _capture_round11_pure_visual(self, union_vis_features: torch.Tensor) -> None:
        if not self.round11_feature_capture_enabled:
            self._round11_pure_union_visual = None
            return
        if union_vis_features.ndim == 4:
            captured = F.adaptive_avg_pool2d(union_vis_features, output_size=1).flatten(1)
        elif union_vis_features.ndim == 2:
            captured = union_vis_features
        else:
            raise ValueError(
                "Round 11 pure union visual must be [E,C,H,W] or [E,D], got "
                f"{tuple(union_vis_features.shape)}"
            )
        self._round11_pure_union_visual = captured.detach()
    ...
    def forward(self, features, proposals: Sequence[BoxList], rel_pair_idxs, OBj=None):
        ...
        if total_unions == 0:
            if self.round11_feature_capture_enabled:
                self._round11_pure_union_visual = feature_ref.new_zeros(
                    (0, int(feature_ref.shape[1]))
                )
            else:
                self._round11_pure_union_visual = None
        ...
        if OBj is not None and hasattr(OBj, "bbox_roi_extractor") and hasattr(OBj, "bbox_head"):
            ...
            self._capture_round11_pure_visual(union_vis_features)
            if self.separate_spatial:
