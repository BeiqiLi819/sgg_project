...
class PairProposalGenerator(nn.Module):
    ...
    def rank_pairs(
        ...
        """Return every pair in deterministic PPG reconstruction-loss order."""

        if not self.loaded:
            raise RuntimeError("PPG ranking requires loaded weights")
        if pair_idx.ndim != 2 or pair_idx.size(1) != 2:
            raise ValueError("pair_idx must have shape [M, 2]")
        if pair_idx.numel() == 0:
            return pair_idx, torch.zeros((0,), dtype=torch.float32)
        if proposal.mode == "xywha":
            losses = self._oriented_losses(proposal, pair_idx)
        else:
            losses = self._horizontal_losses(proposal, pair_idx)
        # First canonicalize endpoints, then use a stable loss sort.  This
        # makes exact ties independent of the incoming candidate enumeration.
        pair_cpu = pair_idx.detach().long().cpu()
        width = max(len(proposal), 1)
        encoded = pair_cpu[:, 0] * width + pair_cpu[:, 1]
        canonical = torch.argsort(encoded, stable=True)
        loss_order = torch.argsort(losses[canonical], stable=True)
        order_cpu = canonical[loss_order]
        order = order_cpu.to(device=pair_idx.device)
        return pair_idx.index_select(0, order), losses.index_select(0, order_cpu)

    def pair_features(
        self, proposal: BoxList, pair_idx: torch.Tensor
    ) -> torch.Tensor:
        """Build the exact released-PPG inference input for arbitrary pairs.

        This is intentionally a feature-construction interface, not a claim
        about the unpublished PPG training procedure.  STAR-Q1 uses it to
        cache immutable inputs for controlled fine-tuning of a copy of the
        released checkpoint.  The returned rows preserve ``pair_idx`` order.
        """

        if pair_idx.ndim != 2 or pair_idx.size(1) != 2:
            raise ValueError("pair_idx must have shape [M, 2]")
        if pair_idx.numel() == 0:
            return proposal.bbox.new_zeros((0, self.input_dim), dtype=torch.float32)
        if proposal.mode == "xywha":
            ...
            head_boxes = obj_boxes[pair_idx[:, 0]]
            tail_boxes = obj_boxes[pair_idx[:, 1]]
            ...
            pair_meta = torch.cat(
                (
                    head_polys,
                    tail_polys,
                    obj_labels[pair_idx[:, 0], None],
                    obj_labels[pair_idx[:, 1], None],
                ),
                dim=1,
            )
            return self._build_oriented_features(
                head_boxes_rad, tail_boxes_rad, pair_meta, proposal.size
            )

        obj_labels = proposal.get_field("labels").long()
        boxes = proposal.convert("xyxy").bbox.to(dtype=torch.float32)
        head = boxes[pair_idx[:, 0]]
        tail = boxes[pair_idx[:, 1]]
        ...
        return torch.cat(
            (
                self._one_hot_labels(obj_labels[pair_idx[:, 0]], pair_idx.device),
                self._one_hot_labels(obj_labels[pair_idx[:, 1]], pair_idx.device),
                spatial,
            ),
            dim=1,
        )

    def reconstruction_losses_from_features(
        self, features: torch.Tensor, *, inference: bool = True
    ) -> torch.Tensor:
        """Return the scalar that the released inference path ranks ascending."""
        ...
        def compute() -> torch.Tensor:
            ...
            return 0.5 * (
                self.criterion(out1, inputs).mean(dim=1)
                + self.criterion(out2, inputs).mean(dim=1)
            )
    ...
    def _oriented_losses(
        ...
        for start in range(0, pair_idx.size(0), self.chunk_size):
            ...
            tail_polys = self._rotated_box_vertices(tail_boxes_rad)
            pair_meta = torch.cat(
                (head_polys, tail_polys, head_labels[:, None], tail_labels[:, None]),
                dim=1,
            )
            features = self._build_oriented_features(
                head_boxes_rad, tail_boxes_rad, pair_meta, img_size
            )
            losses.append(self._compute_chunk_losses(features))
        return torch.cat(losses, dim=0) if losses else torch.zeros((0,))

    def _horizontal_losses(
        self, proposal: BoxList, pair_idx: torch.Tensor
    ) -> torch.Tensor:
        obj_labels = proposal.get_field("labels").long()
        boxes = proposal.convert("xyxy").bbox.to(dtype=torch.float32)
