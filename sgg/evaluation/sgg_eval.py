...
def evaluate_sgg(
    ...
    for pred, target in zip(predictions, targets):
        ...
        best_k = topk[-1]
        best_matched = matched_sets_by_k.get(best_k, set())
        image_predicate_count: Dict[int, int] = {}
        image_predicate_hit: Dict[int, int] = {}
        for relation_index, (_, _, predicate) in enumerate(gt_rows):
            predicate = int(predicate)
            image_predicate_count[predicate] = image_predicate_count.get(predicate, 0) + 1
            if relation_index in best_matched:
                image_predicate_hit[predicate] = image_predicate_hit.get(predicate, 0) + 1
        image_predicate_recall = {
            int(predicate): float(
                image_predicate_hit.get(predicate, 0) / max(count, 1)
            )
            for predicate, count in image_predicate_count.items()
        }
        debug_rows.append(
            {
                ...
                "predicate_recall": image_predicate_recall,
            }
        )
