...
def _is_cuda_device(device: str | torch.device) -> bool:
    """Normalize the two device forms accepted by Trainer."""
...
def _should_save_periodic_checkpoint(
    ...
    """Return whether an optimizer-step checkpoint is due."""
...
def _matched_init_config_sha256(cfg: Dict) -> str:
    ignored = {
        str(value)
        for value in cfg.get("RUNTIME", {}).get(
            "MATCHED_INIT_IGNORE_PATHS", []
        )
    }
    flat: Dict[str, object] = {}

    def visit(value, prefix: str = ""):
        if isinstance(value, dict):
            for key in sorted(value):
                child = f"{prefix}.{key}" if prefix else str(key)
                visit(value[key], child)
        elif prefix not in ignored:
            flat[prefix] = list(value) if isinstance(value, tuple) else value

    visit(cfg)
    encoded = json.dumps(
        flat, sort_keys=True, separators=(",", ":"), default=str
    ).encode("utf-8")
...
def _hpl_head_tail_mean_recall(
    per_predicate_recall: Dict[int, Dict[int, float]],
    predicate_counts: Sequence[int],
    tail_fraction: float,
    tail_class_ids: Sequence[int] = (),
) -> Tuple[Dict[int, float], Dict[int, float], Dict[str, object]]:
    """Compute HPL's frequency-ranked head/tail multilabel mean recall.

    The grouping is defined exclusively by the training predicate histogram.
    Background (index zero) is excluded.  Descending frequency ties retain
    predicate-id order, matching HPLPaperModule's stable ``argsort`` contract.
    """
    ...
    if not 0.0 < fraction <= 1.0:
        raise ValueError(f"HPL tail fraction must be in (0, 1], got {fraction}")

    predicate_ids = list(range(1, len(counts)))
    ranked = sorted(predicate_ids, key=lambda predicate: (-counts[predicate], predicate))
    explicit_tail = [int(value) for value in tail_class_ids]
    if explicit_tail:
        if len(set(explicit_tail)) != len(explicit_tail):
            raise ValueError("HPL evaluator tail IDs must be unique")
        if any(value not in predicate_ids for value in explicit_tail):
            raise ValueError("HPL evaluator tail IDs must be foreground predicates")
        tail_set = set(explicit_tail)
        head_ids = [value for value in ranked if value not in tail_set]
        tail_ids = explicit_tail
        group_source = "hierarchy_tail_class_ids"
    else:
        tail_count = max(1, min(len(ranked), int(round(len(ranked) * fraction))))
        head_ids = ranked[:-tail_count]
        tail_ids = ranked[-tail_count:]
        group_source = "train_predicate_counts"

    def group_mean(ids: Sequence[int]) -> Dict[int, float]:
        values: Dict[int, float] = {}
        for k, recalls in per_predicate_recall.items():
            values[int(k)] = (
                float(sum(float(recalls.get(predicate, 0.0)) for predicate in ids) / len(ids))
                if ids
                else 0.0
            )
        return values
    ...
    return (
        ...
        group_mean(tail_ids),
        {
            ...
            "head_predicates": [int(value) for value in head_ids],
            "tail_predicates": [int(value) for value in tail_ids],
...
def _build_eval_report_lines(
    ...
    if metrics.get("head_mR"):
        lines.append(
            _format_metric_dict_row("head mR", metrics.get("head_mR", {}), topk)
        )
    if metrics.get("tail_mR"):
        lines.append(
            _format_metric_dict_row("tail mR", metrics.get("tail_mR", {}), topk)
        )
...
class Trainer:
    def __init__(
        ...
        print("Optimizer LR contract:", self._optimizer_lr_contract(), flush=True)
        self.global_step = 0
        ...
        self.checkpoint_start_iteration = int(
            cfg["SOLVER"].get("CHECKPOINT_START_ITER", 0)
        )
        ...
        self.initialization_manifest: Dict[str, object] = {}
    ...
    def reset_scheduler_state(self, global_step: int = 0):
        """Rebuild the scheduler while preserving loaded optimizer state.

        A normal checkpoint resume restores the checkpoint's scheduler, including
        its old milestones.  Schedule-extension runs need the current config's
        milestones without discarding SGD momentum accumulated before the branch.
        """
        self.scheduler = self._build_scheduler()
        self.global_step = int(global_step)
        if hasattr(self.scheduler, "last_epoch"):
        ...
        if hasattr(self.scheduler, "get_lr"):
            for param_group, lr in zip(self.optimizer.param_groups, self.scheduler.get_lr()):
                param_group["lr"] = lr
        print(
            "Reset scheduler from current config; preserved optimizer state:",
            {
                "global_step": self.global_step,
                "lr": max(float(group["lr"]) for group in self.optimizer.param_groups),
            },
            flush=True,
        )
    ...
    def _apply_freeze_config(self):
        ...
        trainable_prefixes = tuple(
            str(value)
            for value in model_cfg.get("TRAINABLE_PARAMETER_PREFIXES", [])
            if str(value)
        )
        if trainable_prefixes:
            ...
            if not matched:
                raise ValueError(
                    "MODEL.TRAINABLE_PARAMETER_PREFIXES matched no parameters: "
                    f"{trainable_prefixes}"
                )
            print(
                "Restricted trainable parameters:",
                {
    ...
    def _set_frozen_modules_eval(self):
        ...
        if bool(model_cfg.get("FREEZE_NON_TRAINABLE_MODULES_EVAL", False)):
            frozen_modules = 0
            ...
            if self.global_step == 0:
                print(
                    f"Forced {frozen_modules} fully frozen modules to eval mode.",
                    flush=True,
                )

    def _geo_gate_state(self) -> Dict[str, List[float]]:
        """Collect detached GeoEE gates without coupling Trainer to one predictor."""

        state: Dict[str, List[float]] = {}
        for module_name, module in self.model.named_modules():
            getter = getattr(module, "geo_gate_values", None)
            if not callable(getter):
                continue
            values = [float(value) for value in getter()]
            if values:
                state[module_name or "<root>"] = values
        return state

    def _hprc_adapter_state(self) -> Dict[str, object]:
        """Collect HPRC gates without coupling Trainer to one predictor."""

        state: Dict[str, object] = {}
        for module_name, module in self.model.named_modules():
            getter = getattr(module, "hprc_adapter_diagnostics", None)
            if not callable(getter):
                continue
            values = getter()
            if values.get("hard_enabled") or values.get("universal_enabled"):
                state[module_name or "<root>"] = values
        return state

    def _set_training_sampling_context(self, *, epoch: int) -> None:
        for module in self.model.modules():
            setter = getattr(module, "set_training_sampling_context", None)
            if callable(setter):
                setter(global_step=int(self.global_step), epoch=int(epoch))

    def _matched_sampling_state(self) -> Dict[str, object]:
        state: Dict[str, object] = {}
        for module_name, module in self.model.named_modules():
            getter = getattr(module, "matched_sampling_diagnostics", None)
            if not callable(getter):
                continue
            values = getter()
    ...
    def _build_optimizer(self, model: torch.nn.Module):
        ...
        raw_lr_multipliers = solver_cfg.get("PARAMETER_LR_MULTIPLIERS", {})
        if not isinstance(raw_lr_multipliers, dict):
            raise TypeError("SOLVER.PARAMETER_LR_MULTIPLIERS must be a mapping")
        lr_multipliers = []
        for raw_prefix, raw_multiplier in raw_lr_multipliers.items():
            prefix = str(raw_prefix)
            multiplier = float(raw_multiplier)
            if not prefix:
                raise ValueError("Parameter LR multiplier prefix cannot be empty")
            if not math.isfinite(multiplier) or multiplier <= 0:
                raise ValueError(
                    "Parameter LR multiplier must be finite and positive, got "
                    f"{raw_multiplier!r} for {prefix!r}"
                )
            lr_multipliers.append((prefix, multiplier))
        # A more specific nested prefix takes precedence over its parent.
        lr_multipliers.sort(key=lambda item: len(item[0]), reverse=True)
        # SGG-Toolkit's relation_train_net.py passes IMS_PER_BATCH to
        # make_optimizer(..., rl_factor=...), so every parameter-group LR is
        # BASE_LR * IMS_PER_BATCH.  Prefer that explicit compatibility field;
        # TRAIN_BATCH_SIZE is only a fallback for native configs.  Keeping the
        # switch explicit prevents old configs whose BASE_LR is already the
        # effective LR from being scaled twice.
        scale_by_batch = bool(solver_cfg.get("LR_SCALE_BY_BATCH", True))
        if scale_by_batch:
            if solver_cfg.get("IMS_PER_BATCH") is not None:
                rl_factor = float(solver_cfg["IMS_PER_BATCH"])
            else:
                rl_factor = float(
                    self.cfg.get("DATALOADER", {}).get(
                        "TRAIN_BATCH_SIZE",
                        self.cfg.get("DATALOADER", {}).get("BATCH_SIZE", 1),
                    )
                )
            if rl_factor <= 0:
                raise ValueError(
                    "RPCM batch-scaled LR requires a positive IMS_PER_BATCH "
                    f"or TRAIN_BATCH_SIZE, got {rl_factor}."
                )
        else:
            rl_factor = 1.0
    ...
    def _optimizer_lr_contract(self) -> Dict[str, object]:
        """Return the resolved LR rule for logs/manifests and regression tests."""
        solver_cfg = self.cfg["SOLVER"]
        scale_by_batch = bool(solver_cfg.get("LR_SCALE_BY_BATCH", True))
        if not scale_by_batch:
            factor = 1.0
            source = "disabled"
        elif solver_cfg.get("IMS_PER_BATCH") is not None:
            factor = float(solver_cfg["IMS_PER_BATCH"])
            source = "SOLVER.IMS_PER_BATCH"
        else:
            factor = float(
                self.cfg.get("DATALOADER", {}).get(
                    "TRAIN_BATCH_SIZE",
            ...
            source = "DATALOADER.TRAIN_BATCH_SIZE"
        base_lr = float(solver_cfg["BASE_LR"])
        contract = {
            "base_lr": base_lr,
        ...
        raw_multipliers = solver_cfg.get("PARAMETER_LR_MULTIPLIERS", {})
        if raw_multipliers:
            contract["parameter_lr_multipliers"] = {
                str(prefix): float(multiplier)
                for prefix, multiplier in raw_multipliers.items()
            }
            contract["effective_lr_by_prefix"] = {
                str(prefix): base_lr * factor * float(multiplier)
                for prefix, multiplier in raw_multipliers.items()
            }
        return contract
    ...
    def save_checkpoint(self, epoch: int, name: str = "model_last.pth", metrics: Optional[Dict] = None):
        payload = {
            ...
            "initialization_manifest": dict(self.initialization_manifest),
            "hprc_adapter_state": self._hprc_adapter_state(),
        }
        if metrics is not None:
    ...
    def _trainable_state_sha256(self) -> str:
        """Hash trainable tensors without retaining a second model copy."""
    ...
    def save_model_initialization(
        ...
        *,
        ...
        """Save one shared pre-optimizer model state for matched experiments."""
        ...
        schema = [
            (
                name,
                str(value.dtype) if torch.is_tensor(value) else type(value).__name__,
                tuple(value.shape) if torch.is_tensor(value) else None,
            )
            for name, value in sorted(state.items())
        ]
        payload = {
            "kind": "sgg_exact_model_initialization_v1",
            "model": state,
            ...
            "trainable_state_sha256": self._trainable_state_sha256(),
            "matched_config_sha256": _matched_init_config_sha256(self.cfg),
            "matched_config_ignored_paths": list(
                self.cfg.get("RUNTIME", {}).get(
                    "MATCHED_INIT_IGNORE_PATHS", []
                )
            ),
            "detector_class_channel_order": INTERNAL_DETECTOR_CLASS_ORDER,
    ...
    def load_model_initialization(
        ...
        *,
        ...
        """Strictly load a shared model state without touching solver state."""

        payload = torch.load(path, map_location=self.device)
        if not isinstance(payload, dict) or payload.get("kind") != (
            "sgg_exact_model_initialization_v1"
        ):
            raise ValueError(
                "--init-model-state requires an artifact produced by "
                "--export-init-state, not a training checkpoint"
            )
        artifact_seed = payload.get("seed")
        if expected_seed is not None and artifact_seed != int(expected_seed):
            raise ValueError(
                "Initialization seed mismatch: "
                f"artifact={artifact_seed!r}, runtime={int(expected_seed)!r}"
            )
        expected_contract = _matched_init_config_sha256(self.cfg)
        artifact_contract = str(payload.get("matched_config_sha256", ""))
        if not artifact_contract or artifact_contract != expected_contract:
            raise ValueError(
                "Initialization/config contract mismatch: "
                f"artifact={artifact_contract}, runtime={expected_contract}"
            )
        incompatible = self.model.load_state_dict(payload["model"], strict=True)
        if incompatible.missing_keys or incompatible.unexpected_keys:
            raise RuntimeError(
                "Exact model initialization unexpectedly produced incompatible keys: "
                f"missing={incompatible.missing_keys}, "
                f"unexpected={incompatible.unexpected_keys}"
            )
        actual_hash = self._trainable_state_sha256()
        expected_hash = str(payload.get("trainable_state_sha256", ""))
        if not expected_hash or actual_hash != expected_hash:
            raise RuntimeError(
                "Trainable tensors do not reproduce the shared initialization: "
                f"expected={expected_hash}, actual={actual_hash}"
            )
        self.initialization_manifest = {
            "kind": payload["kind"],
            "path": str(Path(path).resolve()),
            "seed": artifact_seed,
            "state_schema_sha256": payload.get("state_schema_sha256"),
            "trainable_state_sha256": actual_hash,
            "matched_config_sha256": artifact_contract,
        }
        print("Loaded exact model initialization:", self.initialization_manifest, flush=True)
        return dict(self.initialization_manifest)
        ...
        *,
        ...
        if bool(
            self.cfg.get("SOLVER", {}).get(
                "WRITE_VALIDATION_METRICS", False
            )
        ):
            metrics_dir = self.output_dir / "validation_metrics"
            metrics_dir.mkdir(parents=True, exist_ok=True)
            payload = {
                "epoch": int(epoch),
                "global_step": int(self.global_step),
                "split": str(
                    self.cfg.get("SOLVER", {}).get("VAL_SPLIT", "val")
                ),
                "filter_method": str(
                    self.cfg.get("MODEL", {})
                    .get("ROI_RELATION_HEAD", {})
                    .get("TEST_FILTER_METHOD", "")
                ),
                "metrics": metrics,
            }
            metrics_path = metrics_dir / f"epoch_{int(epoch):03d}.json"
            ...
            with (self.output_dir / "validation_history.jsonl").open(
                ...
                handle.flush()
        if not bool(
            self.cfg.get("SOLVER", {}).get("SAVE_BEST_CHECKPOINTS", True)
        ):
            print("Best-metric checkpoint saving is disabled.", flush=True)
            return metrics
        best_metric_names = tuple(
            str(value)
            for value in self.cfg.get("SOLVER", {}).get(
                "BEST_METRICS", ("R", "mR", "HR")
            )
        )
        for metric_name in best_metric_names:
            if metric_name not in self.best_metrics:
                raise ValueError(
                    f"Unsupported SOLVER.BEST_METRICS entry: {metric_name!r}"
                )
    ...
    def load_checkpoint(self, path: str):
        ...
        if isinstance(ckpt, dict) and isinstance(
            ckpt.get("initialization_manifest"), dict
        ):
            self.initialization_manifest = dict(ckpt["initialization_manifest"])
        ...
        optional_missing_prefixes = tuple(
            str(value)
            for value in self.cfg.get("MODEL", {}).get(
                "CHECKPOINT_OPTIONAL_MISSING_PREFIXES", []
            )
            if str(value)
        )
        missing = [
            ...
            and key not in model_init_missing_keys
            and not key.startswith(optional_missing_prefixes)
        ]
        unexpected = [
    ...
    def _load_legacy_rpcm_model_only(
        self,
        source: dict,
        path: str,
        *,
        detector_class_order: str = BACKGROUND_LAST,
    ):
        ...
        for source_key, source_value in source.items():
            ...
            source_has_shape = hasattr(source_value, "shape")
            target_has_shape = hasattr(target_value, "shape")
            # PyTorch state_dicts may contain module extra-state dictionaries
            # in addition to tensors. HPL uses this for its hierarchy/hash
            # audit contract. Preserve compatible non-tensor state instead of
            # treating it as a malformed parameter and dereferencing .shape.
            if not source_has_shape and not target_has_shape:
                update[source_key] = source_value
                used_source_keys.add(source_key)
                continue
            if (
                source_has_shape != target_has_shape
                ...
                skipped_shape.append(
                    (
                        ...
                        tuple(target_value.shape) if hasattr(target_value, "shape") else "<no-shape>",
                    )
                )
                continue
            ...
            if (
                is_detector_classifier_key(source_key)
                and detector_class_order != INTERNAL_DETECTOR_CLASS_ORDER
            ):
                value_to_load = reorder_detector_classifier_rows(
                    source_value,
                    source_order=detector_class_order,
    ...
    def load_rpcm_predictor_weights(self, path: str):
        ...
        if predictor_name.upper() not in supported_predictors:
            raise RuntimeError(
                "--init-rpcm is only supported by an RPCM-compatible route; "
                f"got predictor {predictor_name!r}."
            )
        detector_class_order = (
            str(ckpt.get("detector_class_channel_order"))
            if isinstance(ckpt, dict)
            and ckpt.get("detector_class_channel_order") is not None
            else BACKGROUND_LAST
        )
        return self._load_legacy_rpcm_model_only(
            source,
            path,
            detector_class_order=detector_class_order,
        )

    def load_model_checkpoint_weights(self, path: str):
        """Load a full checkpoint as model-only, preserving new module state.

        Unlike ``load_checkpoint``, this entry point deliberately never reads
        optimizer, scheduler, epoch, or global-step state.  Unlike the legacy
        ``--init-rpcm`` spelling, the contract is explicitly full-model: every
        directly compatible backbone, neck, detector, relation feature
        extractor, GNN, and classifier tensor is restored.  Shape-mismatched
        tensors (for example a newly enabled HPRC predicate buffer) and new
        adapter tensors retain the target model's initialized values.
        """

        checkpoint_path = Path(path)
        if not checkpoint_path.is_file():
            raise FileNotFoundError(f"Model initialization checkpoint not found: {path}")
        digest = hashlib.sha256()
        ...
        expected_sha256 = str(
            self.cfg.get("MODEL", {}).get("INIT_MODEL_CHECKPOINT_SHA256", "")
        ).strip().lower()
        if expected_sha256 and actual_sha256 != expected_sha256:
            raise RuntimeError(
                "Full-model initialization checkpoint SHA256 mismatch: "
        ...
        if not isinstance(source, dict):
            raise TypeError(
                "Full-model initialization checkpoint does not contain a model state dict: "
                f"{path}"
            )
        detector_class_order = (
            str(ckpt.get("detector_class_channel_order"))
            if isinstance(ckpt, dict)
            and ckpt.get("detector_class_channel_order") is not None
            else INTERNAL_DETECTOR_CLASS_ORDER
        )
        report = self._load_legacy_rpcm_model_only(
            source,
            str(checkpoint_path),
            detector_class_order=detector_class_order,
        )
        target_count = len(self.model.state_dict())
        if not report["loaded_predictor"]:
            raise RuntimeError(
                "Full-model initialization did not restore the RPCM predictor"
            )
        load_fraction = len(report["loaded"]) / max(target_count, 1)
        minimum_load_fraction = float(
            self.cfg.get("MODEL", {}).get(
                "INIT_MODEL_CHECKPOINT_MIN_LOAD_FRACTION", 0.0
            )
        )
        if not 0.0 <= minimum_load_fraction <= 1.0:
            raise ValueError(
                "MODEL.INIT_MODEL_CHECKPOINT_MIN_LOAD_FRACTION must be in [0, 1]"
            )
        if load_fraction < minimum_load_fraction:
            raise RuntimeError(
                "Full-model initialization restored too little of the target model: "
                f"loaded={len(report['loaded'])}, target={target_count}, "
                f"fraction={load_fraction:.6f}, required={minimum_load_fraction:.6f}"
            )
        self.initialization_manifest = {
            "kind": "sgg_compatible_full_model_checkpoint_v1",
            "path": str(checkpoint_path.resolve()),
            "sha256": actual_sha256,
            "source_detector_class_channel_order": detector_class_order,
            "loaded_tensors": len(report["loaded"]),
            "target_tensors": target_count,
            "loaded_tensor_fraction": load_fraction,
            "minimum_loaded_tensor_fraction": minimum_load_fraction,
            "new_or_shape_changed_tensors": len(report["unloaded_target"]),
            "optimizer_restored": False,
            "scheduler_restored": False,
            "global_step_restored": False,
        }
        print(
            "Loaded compatible full-model checkpoint with fresh solver state:",
            self.initialization_manifest,
            flush=True,
        )
        return report
    ...
    def train(self, start_epoch: int = 0):
        ...
        validate_on_iter = iteration_compat and bool(
            self.cfg.get("SOLVER", {}).get("VALIDATE_ON_ITER", False)
        )
        val_period_iter = max(
            0,
            int(
        ...
        if validate_on_iter:
            if val_loader is not None and val_period_iter <= 0:
                raise ValueError(
                    "SOLVER.VALIDATE_ON_ITER=True requires a positive "
                    "SOLVER.VAL_PERIOD."
                )
            print(
        ...
        diagnostics_enabled = bool(
            self.cfg.get("SOLVER", {}).get("WRITE_TRAIN_DIAGNOSTICS", False)
        )
        fail_on_nonfinite = bool(
            self.cfg.get("SOLVER", {}).get("FAIL_ON_NONFINITE_LOSS", False)
        )
        diagnostics_path = self.output_dir / "training_diagnostics.jsonl"
        completion_path = self.output_dir / "training_completion.json"
        if diagnostics_enabled and self.global_step == 0:
        ...
        for epoch in range(start_epoch, epochs):
            ...
            for batch_idx, (images, targets, metas) in enumerate(pbar):
                ...
                for meta_index, meta in enumerate(metas):
                    identity = meta.get("image_id")
                    if identity is None:
                        identity = meta.get("file_name", meta.get("filename", meta_index))
                    sample_order.update(str(identity).encode("utf-8"))
                    sample_order.update(b"\0")
                ...
                self._set_training_sampling_context(epoch=epoch)
                ...
                if fail_on_nonfinite and not bool(torch.isfinite(loss).all()):
                    raise FloatingPointError(
                        "Non-finite training loss at "
                        f"epoch={epoch + 1}, batch={batch_idx + 1}, "
                        f"global_step={self.global_step}: {float(loss.detach().cpu())}"
                    )
                ...
                if should_step:
                    if self.grad_norm_clip > 0:
                        ...
                        if fail_on_nonfinite and not math.isfinite(float(grad_total_norm)):
                            raise FloatingPointError(
                                "Non-finite gradient norm at "
                                f"epoch={epoch + 1}, batch={batch_idx + 1}, "
                                f"global_step={self.global_step}"
                            )
                        epoch_grad_norms.append(float(grad_total_norm))
                    self.optimizer.step()
                    self.optimizer.zero_grad(set_to_none=True)
                    self.global_step += 1
                    if (
                        iteration_compat
                        and _should_save_periodic_checkpoint(
                            self.global_step,
                            self.checkpoint_period,
                            self.checkpoint_start_iteration,
                        )
                    ):
                        self.save_checkpoint(
                            epoch + 1,
                ...
                if (
                    ...
                    and self.global_step >= max(val_start_iter, 1)
                    ...
                    iteration_metrics = self._validate_and_checkpoint(
                        val_loader,
                        epoch=epoch + 1,
                        best_recall_k=best_recall_k,
                    )
                    iteration_eval_elapsed += time.perf_counter() - eval_start
                    # evaluate_loader switches every module to eval mode.  The
                    # source RPCM loop resumes relation training immediately
                    # after its iteration-boundary validation.
                    self.model.train()
                    self._set_frozen_modules_eval()
                ...
                if (
                    ...
                    and _is_cuda_device(self.device)
                ):
                    # Historical RPCM does this after every scheduler step.
                    # It has no effect on live tensors, but releases inactive
                    # cuDNN workspaces cached by the CUDA allocator.
            ...
            if epoch_loss_count > 0:
                ...
                geo_gate_state = self._geo_gate_state()
                if geo_gate_state:
                    print(
                        f"epoch {epoch + 1}/{epochs} geo_gate/lambda:",
                        geo_gate_state,
                        flush=True,
                    )
                hprc_adapter_state = self._hprc_adapter_state()
                if hprc_adapter_state:
                    print(
                        f"epoch {epoch + 1}/{epochs} hprc_adapter/gates:",
                        hprc_adapter_state,
                        flush=True,
                    )
                if diagnostics_enabled:
                    grad_tensor = torch.as_tensor(epoch_grad_norms, dtype=torch.float64)
                    diagnostic_row = {
                        "epoch": int(epoch + 1),
                        "global_step": int(self.global_step),
                        "batches": int(epoch_loss_count),
                        "sample_order_sha256": sample_order.hexdigest(),
                        "losses": avg_losses,
                        "learning_rate": float(current_lr),
                        "geo_gate/lambda": geo_gate_state,
                        "hprc_adapter/gates": hprc_adapter_state,
                        "matched_sampling": self._matched_sampling_state(),
                        "gradient_norm": {
                            "count": int(grad_tensor.numel()),
                            "mean": float(grad_tensor.mean()) if grad_tensor.numel() else 0.0,
                            "p95": float(torch.quantile(grad_tensor, 0.95)) if grad_tensor.numel() else 0.0,
                            "max": float(grad_tensor.max()) if grad_tensor.numel() else 0.0,
                        },
                    }
                    with diagnostics_path.open("a", encoding="utf-8") as handle:
                        handle.write(json.dumps(diagnostic_row, sort_keys=True) + "\n")
                        handle.flush()
                completed_epochs += 1
            ...
            if (
                ...
                eval_start = time.perf_counter()
                metrics = self._validate_and_checkpoint(
                    val_loader,
                    epoch=epoch + 1,
                    best_recall_k=best_recall_k,
                )
                eval_elapsed = time.perf_counter() - eval_start
            elif not validate_on_iter and val_loader is not None:
                skip_reasons = []
            ...
            save_last = bool(self.cfg.get("SOLVER", {}).get("SAVE_LAST", True))
            save_last_every_epoch = bool(
                self.cfg.get("SOLVER", {}).get(
                    "SAVE_LAST_EVERY_EPOCH", True
                )
            )
            terminal_epoch = reached_max_iterations or (epoch + 1) >= epochs
            if save_last and (
                (not iteration_compat and save_last_every_epoch)
                or terminal_epoch
            ):
        ...
        if diagnostics_enabled:
            completion = {
                ...
                "configured_max_epochs": int(self.cfg["SOLVER"]["MAX_EPOCHS"]),
                "initialization_manifest": dict(self.initialization_manifest),
                "matched_sampling": self._matched_sampling_state(),
                "hprc_adapter/gates": self._hprc_adapter_state(),
            }
    ...
    def evaluate_loader(
        ...
        self.model.eval()
        for module in self.model.modules():
            reset = getattr(module, "reset_source_weight_statistics", None)
            if callable(reset):
                reset()
        preds_all = []
        gts_all = []
        ...
        rel_cfg = self.cfg["MODEL"]["ROI_RELATION_HEAD"]
        hpl_cfg = rel_cfg.get("HPL", {})
        if bool(hpl_cfg.get("ENABLED", False)):
            head_mean_recall, tail_mean_recall, frequency_groups = (
                _hpl_head_tail_mean_recall(
                    res.per_predicate_recall,
                    rel_cfg.get("PREDICATE_COUNTS", []),
                    float(hpl_cfg.get("TAIL_FRACTION", 0.5)),
                    hpl_cfg.get("TAIL_CLASS_IDS", []),
                )
            )
        else:
            head_mean_recall, tail_mean_recall, frequency_groups = {}, {}, {}
        metrics = {
            ...
            "mR": res.mean_recall,
            "HR": harmonic_recall,
            "head_mR": head_mean_recall,
            "tail_mR": tail_mean_recall,
            "hpl-frequency-groups": frequency_groups,
        ...
        for module in self.model.modules():
            summarize = getattr(module, "source_weight_statistics", None)
            if callable(summarize):
                metrics["source-fusion"] = summarize()
                break
