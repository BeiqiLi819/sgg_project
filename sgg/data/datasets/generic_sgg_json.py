...
class GenericSGGJsonDataset(BaseSGGDataset):
    ...
    def _build_target(
        ...
        # Dataset-native candidate graphs (for example AUG ABS-PRD) are
        # generated offline from training-split statistics only.  Keeping the
        # pairs as an explicit target field lets PredCls copy them into its
        # proposal without consulting validation/test relationship labels.
        if "candidate_pairs" in record:
            candidate_pairs = torch.as_tensor(
                record["candidate_pairs"], dtype=torch.long
            )
            if candidate_pairs.numel() == 0:
                candidate_pairs = candidate_pairs.reshape(0, 2)
            if candidate_pairs.ndim != 2 or candidate_pairs.size(1) != 2:
                raise ValueError(
                    "candidate_pairs must have shape [E,2], got "
                    f"{tuple(candidate_pairs.shape)}"
                )
            if candidate_pairs.numel() > 0:
                if bool((candidate_pairs < 0).any()) or bool(
                    (candidate_pairs >= len(objects)).any()
                ):
                    raise ValueError("candidate_pairs contain an illegal endpoint")
                if bool(
                    (candidate_pairs[:, 0] == candidate_pairs[:, 1]).any()
                ):
                    raise ValueError("candidate_pairs contain a self pair")
                encoded = (
                    candidate_pairs[:, 0] * max(len(objects), 1)
                    + candidate_pairs[:, 1]
                )
                if torch.unique(encoded).numel() != encoded.numel():
                    raise ValueError("candidate_pairs contain a duplicate pair")
            target.add_field("candidate_pairs", candidate_pairs)
