...
def parse_args():
    ...
    parser.add_argument(
        "--rpcm-feat-update-step",
        type=int,
        choices=range(0, 5),
        default=None,
        help="Optional frozen-inference override for Original RPCM propagation steps.",
    )
    parser.add_argument(
        "--rpcm-context-source-mode",
        choices=("all", "annotated", "random"),
        default=None,
        help="Control only the Original RPCM mode-0/1 outgoing relation sources.",
    )
    parser.add_argument(
        "--rpcm-context-source-random-seed",
        type=int,
        default=None,
        help="Deterministic random context-source seed for the random diagnostic.",
    )
    parser.add_argument(
        "--artifact-output-dir",
        type=str,
        default="",
        help="Optional directory for per-image prediction/target artifacts.",
    )
    return parser.parse_args()


def _serialize_boxlist(boxlist, fields: tuple[str, ...]) -> dict:
    payload = {
        "bbox": boxlist.bbox.detach().cpu(),
        "size": tuple(int(value) for value in boxlist.size),
    ...
    for field in fields:
        ...
        payload[field] = value.detach().cpu() if torch.is_tensor(value) else value
    return payload
...
def _write_context_artifacts(
    ...
    *,
    ...
    prediction_fields = (
        ...
        "hpl_refined_mask",
        "hpl_tail_gate",
        "hpl_attention_entropy",
        "hpl_residual_norm_ratio",
        "hpl_matching_cosine_labels",
        "hpl_matching_distance_labels",
        "quota_ppg_topk_pair_idxs",
        "quota_selected_pair_idxs",
        "quota_selected_ppg_ranks",
        "quota_selection_pass",
        "quota_selected_ppg_losses",
    ...
    for index, (prediction, target, meta) in enumerate(
        zip(artifacts["predictions"], artifacts["targets"], artifacts["metas"])
    ):
        image_id = int(meta.get("source_image_id", meta.get("image_id", index)))
        output_path = output_dir / f"{image_id:04d}.pt"
        torch.save(
            {
                "schema_version": 1,
                "image_id": image_id,
                ...
                "prediction": _serialize_boxlist(prediction, prediction_fields),
                "target": _serialize_boxlist(target, target_fields),
            },
            output_path,
        )
        rows.append({"image_id": image_id, "path": output_path.name})
    ...
    print(f"Wrote {len(rows)} context artifacts to {output_dir}", flush=True)
...
def load_model_only_checkpoint(trainer: Trainer, path: str, *, legacy_rpcm: bool = False):
    ...
    for source_key, source_value in source.items():
        if source_key not in target:
            continue
        source_shape = tuple(source_value.shape) if hasattr(source_value, "shape") else None
        target_value = target[source_key]
        target_shape = tuple(target_value.shape) if hasattr(target_value, "shape") else None
        # PyTorch state dicts may contain non-tensor ``*_extra_state`` entries.
        # They are part of the module's checkpoint contract (HPL stores its
        # hierarchy manifest here), so preserve them when both sides are
        # non-tensor values instead of treating them as malformed tensors.
        ...
        if source_shape is None or target_shape is None or source_shape != target_shape:
            skipped_shape.append(
                (
                    source_key,
                    source_key,
                    source_shape if source_shape is not None else "<no-shape>",
                    target_shape if target_shape is not None else "<no-shape>",
                )
            )
            continue
...
def main():
    ...
    if args.rpcm_feat_update_step is not None:
        rel_cfg["RPCM_FEAT_UPDATE_STEP"] = int(args.rpcm_feat_update_step)
    if args.rpcm_context_source_mode is not None:
        rel_cfg["RPCM_CONTEXT_SOURCE_MODE"] = str(
            args.rpcm_context_source_mode
        ).lower()
    if args.rpcm_context_source_random_seed is not None:
        rel_cfg["RPCM_CONTEXT_SOURCE_RANDOM_SEED"] = int(
            args.rpcm_context_source_random_seed
        )
    ...
    cfg["MODEL"]["ROI_BOX_HEAD"]["CLASS_NAMES"] = [
        split_meta.categories[i]
        for i in sorted(split_meta.categories.keys(), key=int)
    ]
    cfg["MODEL"]["ROI_RELATION_HEAD"]["RELATION_NAMES"] = [
        split_meta.predicates[i]
        for i in sorted(split_meta.predicates.keys(), key=int)
    ]
    ...
    if args.checkpoint_load_mode == "full":
        loaded_checkpoint = trainer.load_checkpoint(args.checkpoint)
    elif args.checkpoint_load_mode == "model-only":
        loaded_checkpoint = load_model_only_checkpoint(
            trainer, args.checkpoint, legacy_rpcm=False
        )
    elif args.checkpoint_load_mode == "legacy-rpcm":
        loaded_checkpoint = load_model_only_checkpoint(
            trainer, args.checkpoint, legacy_rpcm=True
        )
    else:
        raise ValueError(f"Unknown checkpoint load mode: {args.checkpoint_load_mode}")

    # Evaluation configs intentionally do not scan the train split. Recover
    # HPL's head/tail grouping prior from the checkpoint that was produced by
    # training, never from val/test labels or their observed frequencies.
    checkpoint_cfg = (
        loaded_checkpoint.get("cfg", {})
        if isinstance(loaded_checkpoint, dict)
        else {}
    )
    checkpoint_train_predicate_counts = (
        checkpoint_cfg.get("MODEL", {})
        .get("ROI_RELATION_HEAD", {})
        .get("PREDICATE_COUNTS", [])
    )
    if checkpoint_train_predicate_counts:
        restored_counts = [
            int(value) for value in checkpoint_train_predicate_counts
        ]
        cfg["MODEL"]["ROI_RELATION_HEAD"]["PREDICATE_COUNTS"] = restored_counts
        trainer.cfg["MODEL"]["ROI_RELATION_HEAD"][
            "PREDICATE_COUNTS"
        ] = restored_counts
            ...
            json.dumps(
                {
                    ...
                    "max_images": int(args.max_images),
                    "filter_method": filter_method,
                    "context_ablation": rel_cfg.get("RPCM_CONTEXT_ABLATION", "full"),
                    "relation_adjacency": rel_cfg.get(
                        "RPCM_RELATION_GRAPH_MODE",
                        rel_cfg.get("RPCM_ORIGINAL_REL_ADJACENCY", "unified"),
                    ),
                    "rpcm_relation_graph_mode": rel_cfg.get(
                        "RPCM_RELATION_GRAPH_MODE"
                    ),
                    "entity_source_fusion": rel_cfg.get(
                        "RPCM_ENTITY_SOURCE_FUSION", "original"
                    ),
                    "rpcm_feat_update_step": int(
                        rel_cfg.get("RPCM_FEAT_UPDATE_STEP", 4)
                    ),
                    "rpcm_context_source_mode": rel_cfg.get(
                        "RPCM_CONTEXT_SOURCE_MODE", "all"
                    ),
                    "rpcm_context_source_random_seed": int(
                        rel_cfg.get("RPCM_CONTEXT_SOURCE_RANDOM_SEED", 0)
                    ),
                    "hpl_diagnostic_eval_route": rel_cfg.get("HPL", {}).get(
                        "DIAGNOSTIC_EVAL_ROUTE", "current"
                    ),
                    "hpl_eval_tail_route": rel_cfg.get("HPL", {}).get(
                        "EVAL_TAIL_ROUTE", "base_prediction"
                    ),
                    "hpl_final_logit_mode": rel_cfg.get("HPL", {}).get(
                        "FINAL_LOGIT_MODE", "original"
                    ),
                    "hpl_final_fusion_weight": float(
                        rel_cfg.get("HPL", {}).get("FINAL_FUSION_WEIGHT", 0.0)
                    ),
                    "train_predicate_counts": [
                        int(value) for value in train_predicate_counts
                    ],
                    "metrics": metrics,
                },
                ensure_ascii=False,
