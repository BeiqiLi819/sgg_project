...
def _sync_model_cfg_from_metadata(cfg: Dict, metadata) -> Dict:
    ...
    if "ROI_RELATION_HEAD" in cfg["MODEL"]:
        # Historical STAR configs store the predicate output dimension under
        # NUM_CLASSES, while newer generic datasets expose NUM_PREDICATES.
        # Synchronize both aliases so a non-STAR catalog cannot silently keep
        # STAR's default 59-way classifier.
        cfg["MODEL"]["ROI_RELATION_HEAD"]["NUM_CLASSES"] = metadata.num_predicates
        cfg["MODEL"]["ROI_RELATION_HEAD"]["NUM_PREDICATES"] = metadata.num_predicates
    return cfg
...
def _build_dataloader_from_dataset(cfg: Dict, dataset, split: str, shuffle: bool = False) -> DataLoader:
    ...
    if runtime_seed is not None:
        split_offset = {"train": 0, "val": 1, "test": 2}.get(
            split.lower(), 3
        )
        generator = torch.Generator()
        generator.manual_seed(int(runtime_seed) + split_offset)
    ...
    aspect_grouping = bool(lcfg.get("ASPECT_RATIO_GROUPING", False))
    if split.lower() == "train" and aspect_grouping:
        bins = [float(value) for value in lcfg.get("ASPECT_RATIO_BINS", [1.0])]
        if bins != sorted(bins):
            raise ValueError("DATALOADER.ASPECT_RATIO_BINS must be sorted")
        aspect_ratios = _dataset_aspect_ratios(dataset)
        group_ids = [bisect.bisect_right(bins, ratio) for ratio in aspect_ratios]
        sampler = (
        ...
        batch_sampler = GroupedBatchSampler(
            ...
            drop_last=bool(lcfg.get("ASPECT_RATIO_DROP_UNEVEN", False)),
        )
...
def _dataset_aspect_ratios(dataset) -> list[float]:
    """Read image aspect ratios without decoding images.

    STAR exposes the source width and height in ``records``.  A small generic
    fallback is retained for datasets implementing the conventional
    ``get_img_info`` interface, but grouping fails loudly when neither audit
    ...
    if records is not None:
        ...
        for index, record in enumerate(records):
            ...
            if width <= 0 or height <= 0:
                raise ValueError(
                    f"Invalid image extent for aspect grouping at index {index}: "
                    f"width={width}, height={height}"
                )
            ratios.append(height / width)
        return ratios

    get_img_info = getattr(dataset, "get_img_info", None)
    if callable(get_img_info):
        ratios = []
        for index in range(len(dataset)):
            info = get_img_info(index)
        ...
        return ratios
    raise TypeError(
        "Aspect-ratio grouping requires dataset.records or dataset.get_img_info()"
    )
