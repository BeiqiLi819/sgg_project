...
class RotatedROIExtractor(nn.Module):
    ...
    def forward(
        ...
        # A frozen rotated detector can still supply the visual backbone for
        # an HBB relation dataset such as AUG.  In that case proposals are
        # ordinary xyxy boxes and must use the HBB pooler even when MMCV's
        # rotated operator is installed.
        all_rotated = all(boxlist.mode == "xywha" for boxlist in proposals)
        if self.obb_pooler is not None and all_rotated:
