...
def select_by_label_pair_quota(
    ...
    *,
    ...
    """Select a fixed-budget prefix using label-pair quota only.

    The input order is the frozen PPG order.  No geometry, degree, topology,
    rarity, or learned score is consulted.  The returned ranks refer to that
    complete input order and ``passes`` uses 0/1/2 for strict/relaxed/fallback.
    """

    if ranked_pairs.ndim != 2 or ranked_pairs.size(1) != 2:
        raise ValueError("ranked_pairs must have shape [M, 2]")
    budget = min(max(int(budget), 0), int(ranked_pairs.size(0)))
    strict_quota = max(int(strict_quota), 0)
    relaxed_quota = max(int(relaxed_quota), strict_quota)
    if budget == 0:
        empty_pairs = ranked_pairs.new_zeros((0, 2))
        empty_values = ranked_pairs.new_zeros((0,))
        return empty_pairs, empty_values, empty_values, {
            "strict_selected": 0,
            "relaxed_selected": 0,
            "fallback_selected": 0,
            "selected": 0,
    ...
    labels_cpu = labels.detach().long().cpu().tolist()
    pairs_cpu = ranked_pairs.detach().long().cpu().tolist()
    selected_ranks: list[int] = []
    selected_set: set[tuple[int, int]] = set()
    label_counts: Dict[tuple[int, int], int] = {}

    def run_pass(quota: int | None) -> int:
        before = len(selected_ranks)
        for rank, raw_pair in enumerate(pairs_cpu):
            if len(selected_ranks) >= budget:
                break
            pair = (int(raw_pair[0]), int(raw_pair[1]))
            if pair in selected_set or pair[0] == pair[1]:
                continue
            label_pair = (int(labels_cpu[pair[0]]), int(labels_cpu[pair[1]]))
            if quota is not None and label_counts.get(label_pair, 0) >= quota:
                continue
            selected_ranks.append(rank)
            selected_set.add(pair)
            label_counts[label_pair] = label_counts.get(label_pair, 0) + 1
        return len(selected_ranks) - before

    strict_count = run_pass(strict_quota)
    relaxed_count = run_pass(relaxed_quota)
    fallback_count = run_pass(None)
    rank_tensor = ranked_pairs.new_tensor(selected_ranks, dtype=torch.long)
    selected = ranked_pairs.index_select(0, rank_tensor)
    pass_tensor = ranked_pairs.new_empty((len(selected_ranks),), dtype=torch.long)
    pass_tensor[:strict_count] = 0
    pass_tensor[strict_count : strict_count + relaxed_count] = 1
    pass_tensor[strict_count + relaxed_count :] = 2
    return selected, rank_tensor, pass_tensor, {
        "strict_selected": int(strict_count),
        "relaxed_selected": int(relaxed_count),
        "fallback_selected": int(fallback_count),
        "selected": int(len(selected_ranks)),
        "budget": int(budget),
    }
...
class RemoteSensingGraphProposalFilter(nn.Module):
    ...
    def __init__(self, cfg: dict):
        ...
        self.ppg = self._make_ppg(cfg) if self.mode in {"HYBRID", "QUOTA_ONLY"} else None
        self.ppn = (
            self._make_ppn(cfg)
    ...
    def filter_pairs(self, proposal: BoxList, pair_idx: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == "QUOTA_ONLY":
            return self._filter_quota_only(proposal, pair_idx)
    ...
    def _filter_quota_only(
        self, proposal: BoxList, pair_idx: torch.Tensor
    ) -> torch.Tensor:
        if self.ppg is None or not self.ppg.loaded:
            raise RuntimeError("RSGP QUOTA_ONLY requires a loaded frozen PPG")
        ranked_pairs, ranked_losses = self.ppg.rank_pairs(proposal, pair_idx)
        selected, selected_ranks, selection_pass, pass_stats = (
            select_by_label_pair_quota(
                ranked_pairs,
                proposal.get_field("labels"),
                budget=self.topk,
                strict_quota=self.label_pair_quota,
                relaxed_quota=self.relaxed_label_pair_quota,
            )
        )
        topk_count = min(int(self.topk), int(ranked_pairs.size(0)))
        proposal.add_field(
            "quota_ppg_topk_pair_idxs", ranked_pairs[:topk_count].detach()
        )
        proposal.add_field("quota_selected_pair_idxs", selected.detach())
        proposal.add_field("quota_selected_ppg_ranks", selected_ranks.detach())
        proposal.add_field("quota_selection_pass", selection_pass.detach())
        proposal.add_field(
            "quota_selected_ppg_losses",
            ranked_losses.index_select(0, selected_ranks.cpu())
            .to(device=proposal.bbox.device)
            .detach(),
        )
        proposal.add_field("ppn_ranked_pair_idxs", ranked_pairs.detach())
        proposal.add_field("degree_capped_pair_idxs", selected.detach())
        proposal.add_field(
            "rsgp_stats",
            {
                **pass_stats,
                "candidate_pool": int(ranked_pairs.size(0)),
                "degree_capped": int(selected.size(0)),
                "topk": int(self.topk),
                "mode": "QUOTA_ONLY",
                "label_pair_quota": int(self.label_pair_quota),
                "relaxed_label_pair_quota": int(self.relaxed_label_pair_quota),
            },
        )
        return selected
