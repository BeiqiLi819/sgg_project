...
class RelationPostProcessor:
    ...
    def __call__(self, x, rel_pair_idxs, proposals: Sequence):
        ...
        for proposal, pair_idx, rel_logit, obj_logit in zip(proposals, rel_pair_idxs, relation_logits, refine_logits):
            ...
            if rel_logit is None or rel_logit.numel() == 0 or pair_idx.numel() == 0:
                ...
                for field in (
                    ...
                    if boxlist.has_field(field):
                        boxlist.add_field(field, boxlist.get_field(field)[:0])
            ...
            boxlist.add_field("pred_rel_logits", rel_logit[sorting_idx])
            ...
            for field in (
                ...
                if boxlist.has_field(field):
                    ...
                    if value.size(0) != sorting_idx.numel():
                        raise RuntimeError(
                            f"HPL diagnostic field {field!r} is not relation-aligned"
                        )
                    boxlist.add_field(field, value[sorting_idx])
            if boxlist.has_field("vehicle_aux_logits"):
