...
class RelationSamplingProcessor:
    def __init__(self, cfg: dict):
        ...
        matched = rel_cfg.get("MATCHED_SAMPLING", {}) or {}
        self.matched_sampling_enabled = bool(matched.get("ENABLED", False))
        self.matched_sampling_seed = int(matched.get("SEED", 1029))
        self.matched_sampling_include_step = bool(matched.get("INCLUDE_STEP", True))
        self._matched_global_step = 0
        self._matched_epoch = 0
        self._matched_trace = hashlib.sha256()
        self._matched_trace_images = 0

    def set_training_sampling_context(self, *, global_step: int, epoch: int) -> None:
        self._matched_global_step = int(global_step)
        self._matched_epoch = int(epoch)

    @staticmethod
    def _image_id(proposal: BoxList, target: BoxList | None, fallback: int) -> int:
        for boxlist in (proposal, target):
            if boxlist is not None and boxlist.has_field("image_id"):
                value = boxlist.get_field("image_id")
                if torch.is_tensor(value) and value.numel() > 0:
                    return int(value.reshape(-1)[0].item())
                return int(value)
        return int(fallback)

    def _local_generator(self, *, image_id: int, stream: int) -> torch.Generator | None:
        if not self.matched_sampling_enabled:
            return None
        step = self._matched_global_step if self.matched_sampling_include_step else 0
        mixed = (
            self.matched_sampling_seed
            + 1_000_003 * (int(image_id) + 1)
            + 97_409 * (int(step) + 1)
            + 65_537 * (self._matched_epoch + 1)
            + 32_771 * (int(stream) + 1)
        ) % (2**63 - 1)
    ...
    def _randperm(
        self, count: int, *, device: torch.device, image_id: int, stream: int
    ) -> torch.Tensor:
        generator = self._local_generator(image_id=image_id, stream=stream)
        if generator is None:
            return torch.randperm(int(count), device=device)
        return torch.randperm(int(count), generator=generator, device="cpu").to(device)
    ...
    def _multinomial(
        ...
        *,
        ...
        generator = self._local_generator(image_id=image_id, stream=stream)
        if generator is None:
            return torch.multinomial(probabilities, samples, replacement=False)
        ...
        return selected.to(probabilities.device)

    def _record_matched_sample(
        self, *, image_id: int, pair_idx: torch.Tensor, labels: torch.Tensor
    ) -> None:
        if not self.matched_sampling_enabled:
            return
        for value in (
            int(image_id),
            self._matched_epoch,
            self._matched_global_step,
            int(pair_idx.size(0)),
        ):
            self._matched_trace.update(int(value).to_bytes(8, "little", signed=True))
        self._matched_trace.update(
            pair_idx.detach().long().cpu().contiguous().numpy().tobytes()
        )
        self._matched_trace.update(
            labels.detach().long().cpu().contiguous().numpy().tobytes()
        )
        self._matched_trace_images += 1

    def matched_sampling_diagnostics(self) -> dict[str, object]:
        return {
            "enabled": self.matched_sampling_enabled,
            "seed": self.matched_sampling_seed,
            "include_step": self.matched_sampling_include_step,
            "sampled_images": self._matched_trace_images,
            "sample_trace_sha256": self._matched_trace.hexdigest(),
            "global_step": self._matched_global_step,
            "epoch": self._matched_epoch,
        }
    ...
    def gtbox_relsample(self, proposals: Sequence[BoxList], targets: Sequence[BoxList]):
        ...
        rel_binarys = []
        out_props = []
        for batch_index, (proposal, target) in enumerate(zip(proposals, targets)):
            image_id = self._image_id(proposal, target, batch_index)
            pair_idx, labels, binary_rel = self._sample_gtbox_rpcm(
                proposal, target, image_id=image_id
            )
            self._record_matched_sample(
                image_id=image_id, pair_idx=pair_idx, labels=labels
            )
    ...
    def label_gtbox_pairs(
        self,
        proposal: BoxList,
        target: BoxList,
        pair_idx: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Label an externally selected PredCls/SGCls pair graph without sampling."""

        if len(proposal) != len(target):
            raise ValueError(
                "GT-box pair labeling requires proposal/target object alignment, "
                f"got {len(proposal)} and {len(target)} objects."
            )
        device = pair_idx.device
        relation_matrix = self._get_relation_matrix(target).to(device)
        if pair_idx.numel() == 0:
        ...
        binary_rel = torch.zeros(
            (len(proposal), len(proposal)), dtype=torch.long, device=device
        )
        positive = torch.nonzero(relation_matrix > 0, as_tuple=False)
        if positive.numel() > 0:
            binary_rel[positive[:, 0], positive[:, 1]] = 1
            binary_rel[positive[:, 1], positive[:, 0]] = 1
        return labels, binary_rel
    ...
    def detect_relsample(self, proposals: Sequence[BoxList], targets: Sequence[BoxList]):
        ...
        for batch_index, (proposal, target) in enumerate(zip(proposals, targets)):
            image_id = self._image_id(proposal, target, batch_index)
            pair_idx, labels, binary_rel = self._sample_detect_rpcm(
                proposal, target, image_id=image_id
            )
            self._record_matched_sample(
                image_id=image_id, pair_idx=pair_idx, labels=labels
    ...
    def _sample_gtbox_rpcm(
        self, proposal: BoxList, target: BoxList, *, image_id: int = 0
    ):
        ...
        if pos_pair_idx.size(0) > num_pos_per_img:
            perm = self._randperm(
                pos_pair_idx.size(0), device=device, image_id=image_id, stream=1
            )[:num_pos_per_img]
        ...
        if num_bg > 0:
            perm = self._randperm(
                bg_pair_idx.size(0), device=device, image_id=image_id, stream=2
            )[:num_bg]
    ...
    def _sample_detect_rpcm(
        ...
        return self._motif_rel_fg_bg_sampling(
            device,
            tgt_rel_matrix,
            ious,
            is_match,
        ...
        for rel_id in range(tgt_rel_labels.size(0)):
            ...
            if fg_rel_i.size(0) > self.num_sample_per_gt_rel:
                ...
                if float(iou_score.sum().item()) > 0:
                    perm = self._multinomial(
                        iou_score / iou_score.sum(),
                        self.num_sample_per_gt_rel,
                        image_id=image_id,
                        stream=1000 + rel_id,
                    )
                else:
