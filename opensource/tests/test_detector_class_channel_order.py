...
def _require_detector_checkpoint():
    path = Path("pretrained/OBB_swin_L_OBD.pth")
    if not path.is_file():
        pytest.skip(f"External detector checkpoint not installed: {path}")
...
class _ExtraStateLoadProbe(nn.Module):
...
def test_model_only_loader_preserves_non_tensor_module_extra_state(tmp_path):
    source_model = _ExtraStateLoadProbe("hpl-hierarchy-v1")
    with torch.no_grad():
        source_model.projection.weight.fill_(3.0)
    checkpoint = tmp_path / "hpl_extra_state.pth"
    torch.save(
        {
            "model": source_model.state_dict(),
            "detector_class_channel_order": BACKGROUND_FIRST,
        },
        checkpoint,
    )

    target_model = _ExtraStateLoadProbe("uninitialized")
    load_model_only_checkpoint(_TrainerProbe(target_model), str(checkpoint))

    assert target_model.manifest == "hpl-hierarchy-v1"
    assert torch.equal(target_model.projection.weight, source_model.projection.weight)
