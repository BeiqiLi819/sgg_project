"""Paper-main STAR OBB SGDet configuration: dual-view RCA + auxiliary LA.

The frozen d1/d2 OBB detector, v5 detection-cache contract, proposal matching,
object refinement, and SGDet label-source protocol come from
``star_sgdet_obb_train.py``. Only the paper-facing relation branch is changed:
the historical HPRC residual head is disabled and auxiliary LA is retained.
"""
...
from configs.star_sgdet_obb_train import cfg as _task_cfg
...
# SGDet predicts object labels on detector proposals.
rel_cfg["OBJECT_REFINE_LOSS_WEIGHT"] = 1.0

# PPG is the checkpoint-selection filter; final PPG/RSGP evaluation reuses
# that exact checkpoint. An unfiltered SGDet graph remains forbidden.
...
cfg["SOLVER"]["OUTPUT_DIR"] = os.environ.get(
    "OUTPUT_DIR", "outputs/paper_submission/sgdet"
)
